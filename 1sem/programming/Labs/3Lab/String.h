...
public:
    ...
    void writeToTextFile(std::ofstream& ofs) const;     // Запись в текстовый файл
    void readFromTextFile(std::ifstream& ifs);          // Чтение из текстового файла
    void writeBinary(std::ofstream& ofs) const;         // Запись в бинарный файл
    void readBinary(std::ifstream& ifs);                // Чтение из бинарного файла
